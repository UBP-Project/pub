...
class CreateUserForm(FlaskForm, UserFormMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.role.choices = [(str(role.id), role.name)
                             for role in Role.query.all() if role.name != 'Administrator']
...
class InterestGroupMixin():
    ...
    about = StringField("About Group", widget=TextArea(),
                        validators=[Required()])
...
class ActivityMixin():
    ...
    description = StringField("Activity Description",
                              widget=TextArea(), validators=[Required()])
...
class CreateActivityForm(FlaskForm, ActivityMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.group.choices = [(None, None)] + [(str(group.id), group.name)
                                               for group in Interest_Group.query.all()]
...
class CreateActivityFormClient(FlaskForm, ActivityMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        groups = Interest_Group.query.join(Membership).join(User)\
            .filter(User.id == current_user.get_id(), Membership.level == 1).all()
