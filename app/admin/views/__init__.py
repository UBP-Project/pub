...
from . import users, activities, groups, perks, points
...
def index():
    upcoming_activities = Activity.query.filter(
        ...
        .order_by(Activity.start_date).limit(4)
    new_users = User.query.order_by(User.timestamp.desc()).limit(6)
    new_groups = Interest_Group.query.order_by(
        Interest_Group.timestamp.desc()).limit(4)
